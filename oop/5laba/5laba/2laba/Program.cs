...
class Program
{
    static void Main(string[] args)
    {
        ...
        try
        {
            ...
            */
            ...
            Console.WriteLine("Введите номер строки, после которой необходимо удалить строки:");
            int lineNumber = Convert.ToInt32(Console.ReadLine());
            Console.WriteLine("Введите количество удаляемых строк:");
            int numberOfLines = Convert.ToInt32(Console.ReadLine());
            int[][] res = DeleteLinesInSteppedArray(lineNumber, numberOfLines, arr3);
    ...
    static int[][] DeleteLinesInSteppedArray(int line, int numberLine, int[][] arr) // удаление строк в рваном массиве после заданной строки
    {
        if (line < arr.GetLength(0))
        {
            if (numberLine < arr.GetLength(0)-line)
            {
                int[][] arr2 = new int[arr.GetLength(0) - numberLine][];

                for (int i = 0; i < line; i++) // копируется количество элементов в строке
                {
                    arr2[i] = new int[arr[i].Length];
                }
                for (int i = line; i < arr2.GetLength(0); i++) // копируется количество элементов в строке
                {
                    arr2[i] = new int[arr[i+numberLine].Length];
                }
                ...
                for (int i = line; i < arr2.GetLength(0); i++)
                {
                    for (int j = 0; j < arr[i + numberLine].Length; j++)
                    {
                        arr2[i][j] = arr[i + numberLine][j];
                    }
                }
